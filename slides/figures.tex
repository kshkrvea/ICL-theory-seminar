...
% Quadratic split (y ~ x^2): × fills the bottom valley (below the U-curve),
% ○ fills the two upper-corner half-spaces (above the U-curve).
% Points are spread across their respective half-spaces, not just in compact
% clusters, to convey that each class occupies a whole region of feature space.
% HOW TO ADJUST:
%   - The U-curve waypoints define the boundary: middle waypoint (0.7,0.5) is
%     the parabola vertex; outer waypoints (-0.15,1.2)/(1.55,1.2) are the arms.
%   - × points must satisfy y < curve(x); ○ points must satisfy y > curve(x).
%   - At x≈0.7 the curve is at y≈0.5, so × lives in the bottom strip there;
%     at x≈0 or 1.4 the curve is at y≈1.1, so × can extend much higher on the
%     sides (the entire lower two-thirds of those columns is × territory).
\newcommand{\hypCurve}{\hypIconBase{
  % half-space fills (clipped to box by \hypIconBase)
  \fill[class0!25] plot[smooth] coordinates {(-0.15,1.2) (0.4,0.55) (0.7,0.5) (1.0,0.55) (1.55,1.2)} -- (1.55,1.55) -- (-0.15,1.55) -- cycle;
  \fill[class1!25] plot[smooth] coordinates {(-0.15,1.2) (0.4,0.55) (0.7,0.5) (1.0,0.55) (1.55,1.2)} -- (1.55,-0.1) -- (-0.15,-0.1) -- cycle;
  % × (class 1) -- below the U, spread across bottom half-space
  \node[class1, font=\tiny] at (0.7,0.18)  {$\times$};
  \node[class1, font=\tiny] at (0.5,0.3)   {$\times$};
  \node[class1, font=\tiny] at (0.9,0.3)   {$\times$};
  \node[class1, font=\tiny] at (0.28,0.55) {$\times$};
  \node[class1, font=\tiny] at (1.12,0.55) {$\times$};
  % ○ (class 0) -- above the U, spread across the two upper-corner regions
  \node[class0, font=\tiny] at (0.1,1.28)  {$\circ$};
  \node[class0, font=\tiny] at (0.3,1.1)   {$\circ$};
  \node[class0, font=\tiny] at (1.1,1.1)   {$\circ$};
  \node[class0, font=\tiny] at (1.3,1.28)  {$\circ$};
  \node[class0, font=\tiny] at (0.12,0.88) {$\circ$};
  \node[class0, font=\tiny] at (1.28,0.88) {$\circ$};
  \draw[testred, thick] plot[smooth] coordinates {(-0.15,1.2) (0.4,0.55) (0.7,0.5) (1.0,0.55) (1.55,1.2)};
}}
...
\newcommand{\metapanel}[5]{%
  \begin{tikzpicture}[scale=0.5]
    ...
    \draw[-{Latex}] (-1.9,-0.2) -- (-1.9,4.2) node[above] {#1};
